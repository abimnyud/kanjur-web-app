...
const ProductGallery: FC<Props> = ({ data: productsData }: Props) => {
    const { meta } = productsData;
    let page: number = meta.page;

    const [data, setData] = useState(productsData.data);
    const [loading, setLoading] = useState(false);

    const getMoreProduct = async (): Promise<void> => {
        setLoading(true);
        let res: any = await fetch(`/api/products?page=${++page}`);
        res = await res.json();
